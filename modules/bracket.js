...
Bracket.prototype.getTeams = function () {
  ...
  return models.Team.find().exec(); //promise
};
  ...
  for (var i=4; i>=0; i--) {
    for (var j=0; j<Math.pow(2,i); j++) {
      ...
      if (i===4) {
        ...
        var game = new Game(i, j%(Math.pow(2,i-1)), Math.ceil(j/8), teamleft1, teamleft2,"b"+i+"-"+j+"-left");
        ...
        var game = new Game(i, j%(Math.pow(2,i-1)), Math.ceil(j/8)+2, teamright1, teamleft2,"b"+i+"-"+j+"-right");
        this.games[i].push(game);
      } else {
