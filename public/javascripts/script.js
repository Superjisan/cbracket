...
Game.prototype.render = function (callback) {
  ...
  $("#"+this.domId+" > .bottom").animate({opacity: "1"},400,"swing",function() {
    $("#"+this.domId+" > .top").animate({opacity: "1"}).complete();
  });
...
Bracket.prototype.play = function(playFunc) {
  ...
  for(var round=startRound; round >= 0; round--) {
    ...
    for(var i=0; i < arr.length; i+=2) {
      ...
      // if (winnerteam1.name == "UTEP") {
      //   debugger;
      // }
      
      winner2 = game2.play(playFunc);
      winnerteam2 = game2['team'+winner2];
      if(winnerteam1.pts_against===1816) console.log('team1 ', round, ", ", winnerteam1);
      if(winnerteam2.pts_against===1816) console.log('team2 ', round, ", ", winnerteam2);
  ...
  for (var i=4; i>=0; i--) {
    for (var j=0; j<Math.pow(2,i); j++) {
      if(i===4) {
        ...
        // var game = new Game(i, j%(Math.pow(2,i-1)), Math.ceil((j+1)/8), teamleft1, teamleft2,"b"+i+"-"+j+"-left");
